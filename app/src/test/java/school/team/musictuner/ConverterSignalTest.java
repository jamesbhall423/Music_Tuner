...
public class ConverterSignalTest {
    ...
    private void testSignal(Converter converter, Pitch[] input, Pitch[] noise, Sound storage, double[] phaseShift,int on, int off) {
        for (int i = 0; i < input.length; i++) for (int time = 0; time < storage.length(); time++)
            storage.setDataAt(time,storage.getDataAt(time)+input[i].getAmplitude()*Math.sin(2*Math.PI*input[i].getFrequency()*time/storage.samplesPerSecond()+phaseShift[i]));
        for (int i = 0; i < noise.length; i++) for (int time = 0; time < storage.length(); time++)
            storage.setDataAt(time,storage.getDataAt(time)+noise[i].getAmplitude()*Math.sin(2*Math.PI*noise[i].getFrequency()*time/storage.samplesPerSecond()+phaseShift[i]));
        Pitch[] output = converter.getSignal(storage,on,off).frequencies.toArray(new Pitch[0]);
        printPitches("input",input);
        printPitches("noise",noise);
        printPitches("output",output);
        double[] signal = new double[off-on];
        for (int i = 0; i < signal.length; i++) signal[i]=storage.getDataAt(i+on);
        testTransform(signal,100,storage.samplesPerSecond());
    ...
    private void printPitches(String base, Pitch[] pitches) {
        ...
        for (int i = 0; i < pitches.length; i++) System.out.println(pitches[i]);
    }
    //0<=phaseShift<1
    ...
    public void testConverterFFTLength() {
        ...
        System.out.println("Input length: "+real.length+" output length: "+length);
    }
    @Test
    public void testConverterActualFrequencies() {
        double[] fft = new double[] {1,2,1,3,1};
        Converter converter = new Converter();
        Settings settings = new Settings();
        settings.setSensitivity(0.5);
        converter.setSettings(settings);
        double[] out = converter.actualFrequencies(fft,100);
        for (int i = 0; i < out.length; i++) System.out.println(out[i]);
    }
    @Test
    public void testConverterFFT() {
        double[] input = new double[] {-1,1,-1,1,-1,1,-1,1};
        double[] input2 = new double[8];
        double[] output = Converter.fft(input,input2,true);
        System.out.println("output.length: "+output.length);
        for (int i = 0; i < output.length; i++) System.out.println(output[i]);
        double[] temp = input;
        input=input2;
        input2=temp;
        output = Converter.fft(input,input2,true);
        System.out.println();
        for (int i = 0; i < output.length; i++) System.out.println(output[i]);
        input = new double[] {0,-1,0,1,0,-1,0,1};
        input2 = new double[8];
        output = Converter.fft(input,input2,true);
        System.out.println();
        for (int i = 0; i < output.length; i++) System.out.println(output[i]);
